...
class GenericEstimator(Estimator):
    """
    An `Estimator` type with utilities for creating models in any framework
    ...
    @classmethod
    def from_hyperparameters(cls, **hyperparameters) -> "GenericEstimator":
        Model = getattr(cls.__init__, "Model", None)
    ...
    def create_training_network(self) -> GenericNetwork:
        """
        Create and return the network used for training (i.e., computing the
        loss).
    ...
    def create_predictor(
        self, transformation: Transformation, trained_network: GenericNetwork
    ) -> Predictor:
        """
        Create and return a predictor object.
    ...
    def train_model(
        ...
        untrained_net = self.create_training_network()
        ...
        training_data_loader = TrainDataLoader(
            ...
            batch_size=self.trainer.batch_size,
            num_batches_per_epoch=self.trainer.num_batches_per_epoch,
            batchify_fn=untrained_net.get_batchify_fn(batchify_fn=batchify_fn),
            num_workers=num_workers,
            num_prefetch=num_prefetch,
            shuffle_buffer_length=shuffle_buffer_length,
        )
        ...
        if validation_data is not None:
            validation_data_loader = ValidationDataLoader(
                ...
                transform=transformation,
                batch_size=self.trainer.batch_size,
                batchify_fn=untrained_net.get_batchify_fn(
